...
struct OwnedChange {
    ...
    esdb_object_change_operation operation = ESDB_OBJECT_CHANGE_PUT;
...
esdb_status read_store_revision_metadata(
    ...
    if (!database || !database->handle || !out_revision) {
        return esdb_detail::fail(
            database, error, ESDB_ERR_INVALID_ARGUMENT, ESDB_PHASE_OBJECT_STORE,
    ...
    esdb_status status = statement.prepare(
        ...
        " SELECT seq FROM sqlite_sequence WHERE name='__esdb_object_changes'"
        "),0) "
        "FROM __esdb_meta m WHERE m.key='store_revision';",
        ESDB_PHASE_OBJECT_STORE,
        error);
    if (status != ESDB_OK) return status;

    int step = 0;
    status = statement.step(database, &step, ESDB_PHASE_OBJECT_STORE, error);
    if (status != ESDB_OK) return status;
    if (step != SQLITE_ROW) {
        return esdb_detail::fail(
            database, error, ESDB_ERR_CORRUPT, ESDB_PHASE_OBJECT_STORE,
...
esdb_status inspect_store_table_count(
    ...
    esdb_status status = tables.prepare(
        ...
        "'__esdb_meta','__esdb_stores','__esdb_records','__esdb_object_changes'"
        ");",
        ESDB_PHASE_OBJECT_STORE,
        error);
    if (status != ESDB_OK) return status;
    ...
    int step = 0;
    status = tables.step(database, &step, ESDB_PHASE_OBJECT_STORE, error);
    if (status != ESDB_OK) return status;
    if (step != SQLITE_ROW) {
        return esdb_detail::fail(
            database, error, ESDB_ERR_SQLITE, ESDB_PHASE_OBJECT_STORE,
...
esdb_status require_store_writable(
    ...
    if (read_only < 0) {
        return esdb_detail::fail(
            database, error, ESDB_ERR_INTERNAL, ESDB_PHASE_OBJECT_STORE,
            database->handle, SQLITE_ERROR,
            "SQLite main database was not found");
    }
    if (read_only != 0) {
        return esdb_detail::fail(
            database, error, ESDB_ERR_IO, ESDB_PHASE_OBJECT_STORE,
...
esdb_status begin_store_mutation(
    ...
    const esdb_status status =
        esdb_detail::exec_sql(database, sql, ESDB_PHASE_OBJECT_STORE, error);
    if (status != ESDB_OK) return status;
...
void rollback_store_mutation(
    ...
    if (scope->owns_transaction) {
        if (esdb_detail::exec_sql(
                database, "ROLLBACK;", ESDB_PHASE_OBJECT_STORE, nullptr) == ESDB_OK) {
...
esdb_status commit_store_mutation(
    ...
    if (!scope || !scope->active) {
        return esdb_detail::fail(
            database, error, ESDB_ERR_INVALID_STATE, ESDB_PHASE_OBJECT_STORE,
            database ? database->handle : nullptr, SQLITE_MISUSE,
...
esdb_status set_store_revision(
    ...
    status = statement.bind_int64(
        database, 1, revision, ESDB_PHASE_OBJECT_STORE, error);
    if (status != ESDB_OK) return status;
...
esdb_status get_store_id(
    ...
    if (create) {
        ...
        status = insert.bind_text(
            database, 1, name, -1, ESDB_PHASE_OBJECT_STORE, error);
        if (status != ESDB_OK) return status;
        status = insert.bind_int64(
            database, 2, esdb_detail::unix_time_ms(),
            ESDB_PHASE_OBJECT_STORE, error);
        if (status != ESDB_OK) return status;

        int step = 0;
        status = insert.step(database, &step, ESDB_PHASE_OBJECT_STORE, error);
        if (status != ESDB_OK) return status;
        if (step != SQLITE_DONE) {
            return esdb_detail::fail(
                database, error, ESDB_ERR_SQLITE, ESDB_PHASE_OBJECT_STORE,
    ...
    int step = 0;
    status = query.step(database, &step, ESDB_PHASE_OBJECT_STORE, error);
    if (status != ESDB_OK) return status;
...
esdb_status insert_change(
    ...
    esdb_status status = statement.prepare(
        database,
        "INSERT INTO __esdb_object_changes("
    ...
    status = statement.bind_int64(
        database, 1, store_id, ESDB_PHASE_OBJECT_STORE, error);
    if (status != ESDB_OK) return status;
    status = statement.bind_text(
        database, 2, key, -1, ESDB_PHASE_OBJECT_STORE, error);
    if (status != ESDB_OK) return status;
    ...
    if (status != ESDB_OK) return status;
    status = statement.bind_int64(
        database, 5, changed_at_ms, ESDB_PHASE_OBJECT_STORE, error);
    if (status != ESDB_OK) return status;
...
esdb_status clone_sql_value(
    ...
    if (!value) {
        return esdb_detail::fail(
            database, error, ESDB_ERR_OUT_OF_MEMORY, ESDB_PHASE_OBJECT_STORE,
...
esdb_status ensure_object_store_schema(
    esdb_database *database,
    esdb_error *error) noexcept {
    clear_error(error);
    if (!database || !database->handle) {
    ...
    std::lock_guard<esdb_detail::NoThrowMutex> schema_lock(
        database->object_store_schema_mutex);
    if (database->object_store_schema_ready) return ESDB_OK;
    ...
    if (store_table_count == 0) {
        if (read_only != 0) {
            return fail(
                database, error, ESDB_ERR_NOT_FOUND, ESDB_PHASE_OBJECT_STORE,
        ...
        if (store_table_count == 0) {
            const char *schema_sql =
                ...
                ") WITHOUT ROWID;"
                "CREATE TABLE __esdb_object_changes("
                ...
                " FOREIGN KEY(store_id) REFERENCES __esdb_stores(id) ON DELETE CASCADE"
                ");"
                "CREATE INDEX __esdb_object_changes_store_revision "
                " ON __esdb_object_changes(store_id, revision);"
            ...
            status = exec_sql(
                database, schema_sql, ESDB_PHASE_OBJECT_STORE, error);
            if (status != ESDB_OK) {
                rollback_store_mutation(database, &schema_scope);
                return status;
            }
    ...
    if (!version || std::strcmp(version, "1") != 0) {
        return fail(
            database, error, ESDB_ERR_UNSUPPORTED, ESDB_PHASE_OBJECT_STORE,
    ...
    database->object_store_schema_ready = true;
    return ESDB_OK;
}
...
esdb_status object_store_changes_since_impl(
    ...
    if (!database || !database->handle || !callback ||
        limit > ESDB_OBJECT_CHANGE_LIMIT_MAX) {
        return fail(
    ...
    if (store_name_or_null && !valid_object_store_name(store_name_or_null)) {
        return fail(
            database, error, ESDB_ERR_INVALID_ARGUMENT,
            ESDB_PHASE_SUBSCRIPTION,
            database->handle, SQLITE_MISMATCH,
    ...
    if (limit == 0u) limit = ESDB_OBJECT_CHANGE_LIMIT_MAX;
    ...
    try {
...

        {
            ...
            std::lock_guard<esdb_detail::NoThrowMutex> store_lock(
                database->object_store_write_mutex);

            esdb_status status = ensure_object_store_schema(database, error);
            if (status != ESDB_OK) return status;

            const char *sql =
                store_name_or_null
                    ? "SELECT c.revision,c.operation,c.value_type,s.name,c.key "
                      "FROM __esdb_object_changes c "
            ...
            while (true) {
                ...
                const auto operation = static_cast<esdb_object_change_operation>(
                ...
                if (raw_revision <= 0 ||
                    !store_name || !key ||
                    !valid_object_store_name(store_name) ||
                    !valid_object_store_key(key) ||
                    !valid_value_type(static_cast<int>(value_type)) ||
                    (operation != ESDB_OBJECT_CHANGE_PUT &&
                     operation != ESDB_OBJECT_CHANGE_DELETE) ||
                    (operation == ESDB_OBJECT_CHANGE_DELETE &&
...
esdb_status esdb_object_store_ensure(
    esdb_database *database,
    ...
    if (!database || !database->handle ||
        !esdb_detail::valid_object_store_name(store_name_utf8)) {
        return esdb_detail::fail(
            database, error, ESDB_ERR_INVALID_ARGUMENT, ESDB_PHASE_OBJECT_STORE,
    ...
    std::lock_guard<esdb_detail::NoThrowMutex> write_lock(
        database->object_store_write_mutex);
    esdb_status status = esdb_detail::ensure_object_store_schema(database, error);
    if (status != ESDB_OK) return status;
...
esdb_status esdb_object_store_put(
    esdb_database *database,
    const char *store_name_utf8,
    const char *key_utf8,
    ...
    if (!database || !database->handle ||
        !esdb_detail::valid_object_store_name(store_name_utf8) ||
        !esdb_detail::valid_object_store_key(key_utf8) ||
        !value ||
    ...
    esdb_status status =
        esdb_detail::ensure_object_store_schema(database, error);
    if (status != ESDB_OK) return status;
    status = require_store_writable(database, error);
    if (status != ESDB_OK) return status;
    ...
    if (status == ESDB_OK) {
        status = record.bind_text(
            database, 2, key_utf8, -1, ESDB_PHASE_OBJECT_STORE, error);
    }
    if (status == ESDB_OK) {
    ...
    if (status == ESDB_OK) {
        if (value->payload.empty()) {
            status = record.bind_zeroblob(
                database, 4, 0, ESDB_PHASE_OBJECT_STORE, error);
        } else {
    ...
    if (status == ESDB_OK) {
        status = record.bind_int64(
            database, 5, revision, ESDB_PHASE_OBJECT_STORE, error);
    }
    if (status == ESDB_OK) {
        status = record.bind_int64(
            database, 6, now, ESDB_PHASE_OBJECT_STORE, error);
    }
    if (status != ESDB_OK) {
        rollback_store_mutation(database, &mutation);
        return status;
    ...
    int step = 0;
    status = record.step(database, &step, ESDB_PHASE_OBJECT_STORE, error);
...
esdb_status esdb_object_store_get(
    esdb_database *database,
    const char *store_name_utf8,
    const char *key_utf8,
    esdb_value **out_value,
    ...
    if (status != ESDB_OK) return status;
    status = statement.bind_text(
        database, 1, store_name_utf8, -1, ESDB_PHASE_OBJECT_STORE, error);
    if (status != ESDB_OK) return status;
    status = statement.bind_text(
        database, 2, key_utf8, -1, ESDB_PHASE_OBJECT_STORE, error);
    if (status != ESDB_OK) return status;
    ...
    if (step == SQLITE_DONE) {
        /*
         * A key miss is normal object-store control flow, not a database
         * health failure. The caller already receives ESDB_ERR_NOT_FOUND and
         * the output remains NULL; do not poison last-error/error_count.
         */
...
esdb_status esdb_object_store_delete(
    esdb_database *database,
    const char *store_name_utf8,
    const char *key_utf8,
    int *out_deleted,
    ...
    int step = 0;
    status = remove.step(database, &step, ESDB_PHASE_OBJECT_STORE, error);
    if (status != ESDB_OK) {
        rollback_store_mutation(database, &mutation);
        return status;
    }
...
esdb_status esdb_object_store_exists(
    esdb_database *database,
    const char *store_name_utf8,
    const char *key_utf8,
    int *out_exists,
...
esdb_status esdb_object_store_count(
    esdb_database *database,
    const char *store_name_utf8,
    uint64_t *out_count,
    esdb_error *error) {
...
esdb_status esdb_object_store_scan(
    esdb_database *database,
    const char *store_name_utf8,
    ...
    if (!database || !database->handle || !callback ||
        ...
        (after_key_or_null_utf8 &&
         !esdb_detail::valid_object_store_key(after_key_or_null_utf8)) ||
        limit > ESDB_OBJECT_SCAN_LIMIT_MAX) {
        return esdb_detail::fail(
            database, error, ESDB_ERR_INVALID_ARGUMENT, ESDB_PHASE_OBJECT_STORE,
            database ? database->handle : nullptr, SQLITE_MISUSE,
            "invalid Store scan arguments");
    }
    if (limit == 0u) limit = ESDB_OBJECT_SCAN_LIMIT_MAX;
    ...
    try {
...

        {
            ...
            const char *sql = after_key_or_null_utf8
                ? "SELECT r.key,r.type,r.value,r.revision "
                  "FROM __esdb_records r "
                  "JOIN __esdb_stores s ON s.id=r.store_id "
                  "WHERE s.name=?1 AND r.key>?2 "
                  "ORDER BY r.key COLLATE BINARY ASC LIMIT ?3;"
                : "SELECT r.key,r.type,r.value,r.revision "
                  "FROM __esdb_records r "
                  "JOIN __esdb_stores s ON s.id=r.store_id "
                  "WHERE s.name=?1 "
                  "ORDER BY r.key COLLATE BINARY ASC LIMIT ?2;";

            esdb_detail::Statement statement;
            status = statement.prepare(
                database, sql, ESDB_PHASE_OBJECT_STORE, error);
            if (status != ESDB_OK) return status;
            status = statement.bind_text(
                database, 1, store_name_utf8, -1, ESDB_PHASE_OBJECT_STORE, error);
            if (status != ESDB_OK) return status;
            if (after_key_or_null_utf8) {
                status = statement.bind_text(
                    database, 2, after_key_or_null_utf8, -1,
                    ESDB_PHASE_OBJECT_STORE, error);
                if (status != ESDB_OK) return status;
                status = statement.bind_int(
                    database, 3, static_cast<int>(limit),
            ...
            while (true) {
                int step = 0;
                status = statement.step(
                    database, &step, ESDB_PHASE_OBJECT_STORE, error);
                if (status != ESDB_OK) return status;
                if (step == SQLITE_DONE) break;

                const char *key = reinterpret_cast<const char *>(
                    sqlite3_column_text(statement.get(), 0));
                const int key_bytes = sqlite3_column_bytes(statement.get(), 0);
                const sqlite3_int64 raw_revision =
                    sqlite3_column_int64(statement.get(), 3);
                if (!key || key_bytes <= 0 ||
                    static_cast<std::size_t>(key_bytes) != std::strlen(key) ||
                    !esdb_detail::valid_object_store_key(key) ||
                    raw_revision <= 0) {
                    return esdb_detail::fail(
                        database, error, ESDB_ERR_CORRUPT, ESDB_PHASE_OBJECT_STORE,
                        database->handle, SQLITE_CORRUPT,
                        "Store scan encountered an invalid record");
                }

                OwnedRecord owned;
                owned.key.assign(key, static_cast<std::size_t>(key_bytes));
                owned.revision = static_cast<std::uint64_t>(raw_revision);

                esdb_value *cloned = nullptr;
                status = clone_sql_value(
                    statement.get(), 1, 2, &cloned, database, error);
                if (status != ESDB_OK) return status;
                owned.value.type = cloned->type;
                owned.value.payload = std::move(cloned->payload);
                delete cloned;
                snapshot.emplace_back(std::move(owned));
            }
        }
        ...
        return esdb_detail::fail(
            database, error, ESDB_ERR_INTERNAL, ESDB_PHASE_OBJECT_STORE,
            database->handle, SQLITE_ERROR,
            "unexpected exception while building Store scan snapshot");
    }

    std::uint32_t delivered = 0u;
    for (OwnedRecord &owned : snapshot) {
        esdb_object_record record{};
...
esdb_status esdb_object_store_revision(
    esdb_database *database,
    uint64_t *out_revision,
    esdb_error *error) {
    esdb_detail::count_operation(database);
...
esdb_status esdb_object_store_changes_since(
    esdb_database *database,
    ...
    esdb_error *error) {
    esdb_detail::count_operation(database);
    return esdb_detail::object_store_changes_since_impl(
        database,
...
esdb_status esdb_object_store_prune_changes(
    esdb_database *database,
    ...
    if (through_revision > ESDB_REVISION_MAX) {
        return invalid_revision(
            database, error, ESDB_PHASE_OBJECT_STORE,
    ...
    status = statement.prepare(
        database,
        "DELETE FROM __esdb_object_changes WHERE revision<=?1 RETURNING revision;",
        ESDB_PHASE_OBJECT_STORE,
        error);
    if (status != ESDB_OK) return status;
    status = statement.bind_int64(
...
esdb_status esdb_object_subscribe(
    esdb_database *database,
    const char *store_name_or_null_utf8,
    uint64_t after_revision,
    ...
    if (!database || !database->handle || !out_subscription ||
        (store_name_or_null_utf8 &&
         !esdb_detail::valid_object_store_name(store_name_or_null_utf8))) {
        return esdb_detail::fail(
            database, error, ESDB_ERR_INVALID_ARGUMENT,
            ESDB_PHASE_SUBSCRIPTION,
            database ? database->handle : nullptr, SQLITE_MISUSE,
    ...
    esdb_object_subscription *subscription =
        new (std::nothrow) esdb_object_subscription();
    if (!subscription) {
        return esdb_detail::fail(
            database, error, ESDB_ERR_OUT_OF_MEMORY,
            ESDB_PHASE_SUBSCRIPTION,
...
esdb_status esdb_object_subscription_poll(
    esdb_object_subscription *subscription,
    uint32_t limit,
    esdb_object_change_callback callback,
    void *user_data,
    ...
    const esdb_status status =
        esdb_detail::object_store_changes_since_impl(
...
uint64_t esdb_object_subscription_revision(
    const esdb_object_subscription *subscription) {
    return subscription
        ? subscription->revision.load(std::memory_order_relaxed)
        : 0u;
}

void esdb_object_subscription_destroy(esdb_object_subscription *subscription) {
    delete subscription;
}
